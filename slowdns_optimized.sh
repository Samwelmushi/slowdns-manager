...
create_service() {
    ...
    # ── dnstt-server flags ────────────────────────────────────────────────────
    # -udp :5300          — listen on UDP port 5300 (NAT-redirected from 53)
    # -privkey-file       — server private key
    # -mtu $mtu           — MUST match exactly what the client uses; at 512 this
    #                       controls the max DNS TXT/CNAME payload per query.
    #
    # GOMAXPROCS=0        — let Go runtime auto-detect CPU count (better than
    #                       hardcoding 4; on single-vCPU VPS, 4 caused overhead).
    # GODEBUG=netdns=go   — use Go's pure-Go DNS resolver (avoids cgo overhead).
    # GOGC=200            — run GC less frequently; at 512B pkt rate the GC
    #                       was triggering every ~50 ms and causing latency spikes.
    #                       200 = allow heap to grow 2× before GC → fewer pauses.
    # GOMEMLIMIT          — hard cap on Go heap; prevents OOM on small VPS.
    # ─────────────────────────────────────────────────────────────────────────
...
# ── Go runtime tuning ──────────────────────────────────────────────────────
# GOMAXPROCS=0: auto-detect vCPU count (avoids over-threading on 1-vCPU VPS)
# GODEBUG=netdns=go: pure-Go resolver, no cgo latency
# GOGC=200: GC runs half as often → fewer pause-spikes at high packet rate
# GOMEMLIMIT: cap heap at 512 MB to protect VPS from OOM
...
# ── I/O multiplexing: maximum file descriptors ─────────────────────────────
# Each active DNS stream consumes one FD. At 2–4 Mbps with 512B packets
# there can be hundreds of concurrent streams. Set the ceiling high.
LimitNOFILE=1048576
LimitNPROC=65536

# ── CPU scheduler: high priority, not real-time ───────────────────────────
# nice=-15 gives the process scheduling priority over most other daemons
# without locking it into FIFO/RR which can starve kernel threads on 1-vCPU.
Nice=-15
...
EOF
    ...
    echo -e "${GREEN}✓ Service created (512B high-packet-rate settings)${NC}"
    log_success "DNSTT Configuration:"
    log_message "   MTU:          $mtu bytes"
    log_message "   SSH Port:     $ssh_port"
    log_message "   UDP Port:     5300"
    log_message "   GOMAXPROCS:   auto"
    log_message "   GOGC:         200 (reduced GC pause)"
    log_message "   LimitNOFILE:  1M FDs"
    log_message "   Nice:         -15 (high priority)"
    sleep 2
}
