...
ExecStart=$DNSTT_SERVER -udp :5300 -privkey-file $INSTALL_DIR/server.key -mtu $mtu $tunnel_domain 127.0.0.1:7100
Restart=always
RestartSec=3
...
EOF
    ...
    log_message "   CPU Priority: Realtime FIFO 99"
    log_message "   I/O Priority: Realtime"
    log_message "   Nice: -20 (highest)"
    log_message "   CPU Quota: 1600% (16 cores)"
    log_message "   File Descriptors: 1M"
    log_message "   MTU: $mtu bytes"
    log_message "   SSH Forwarding: 127.0.0.1:7100"
    sleep 2
}
...
setup_dnstt() {
    ...
    echo -e "${WHITE}🔗 SSH Forward:${NC}     ${YELLOW}127.0.0.1:7100${NC}"
    ...
    echo ""
    echo -e "${YELLOW}📱 CLIENT COMMAND (ULTRA SPEED):${NC}"
    echo ""
    echo -e "${CYAN}Cloudflare DoH (Recommended):${NC}"
    echo -e "${WHITE}dnstt-client -doh https://cloudflare-dns.com/dns-query \\${NC}"
    echo -e "${WHITE}  -pubkey $PUBKEY \\${NC}"
    echo -e "${WHITE}  -mtu $MTU \\${NC}"
    echo -e "${WHITE}  $tunnel_domain 127.0.0.1:7100${NC}"
    echo ""
    echo -e "${CYAN}Google DoH:${NC}"
    ...
    echo -e "   ${GREEN}✓${NC} 4M connection tracking"
    echo -e "   ${GREEN}✓${NC} Realtime priority (FIFO 99)"
    echo -e "   ${GREEN}✓${NC} TCP FastOpen"
    echo -e "   ${GREEN}✓${NC} DNS-specific optimizations"
    echo -e "   ${GREEN}✓${NC} SSH Forwarding: 127.0.0.1:7100"
    echo ""
...
dnstt-client -doh https://cloudflare-dns.com/dns-query -pubkey $PUBKEY -mtu $MTU $tunnel_domain 127.0.0.1:7100

# Google DoH
dnstt-client -doh https://dns.google/dns-query -pubkey $PUBKEY -mtu $MTU $tunnel_domain 127.0.0.1:7100

# Quad9 DoH
dnstt-client -doh https://dns.quad9.net/dns-query -pubkey $PUBKEY -mtu $MTU $tunnel_domain 127.0.0.1:7100

# Direct UDP (Alternative)
dnstt-client -udp $PUBLIC_IP:5300 -pubkey $PUBKEY -mtu $MTU $tunnel_domain 127.0.0.1:7100

SSH TUNNELING THROUGH DNSTT:
═════════════════════════════
# After running dnstt-client, connect SSH through the tunnel:
ssh -D 1080 -p 7100 username@127.0.0.1

# Or use as SOCKS5 proxy:
# Set your browser/app to use SOCKS5 proxy at 127.0.0.1:1080
...
✓ SSH Forwarding on 127.0.0.1:7100
