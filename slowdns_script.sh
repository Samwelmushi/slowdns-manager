...
check_for_updates() {
    ...
    scanning_effect "${CYAN}Checking for updates${NC}"
    echo ""
    
    echo -e "${WHITE}Current Version: ${GREEN}v$SCRIPT_VERSION${NC}"
    echo ""
    
    # Note: This is a placeholder - you need to set up your own GitHub repo
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${CYAN}To enable automatic updates:${NC}"
    echo ""
    echo -e "  ${WHITE}1.${NC} Upload this script to your GitHub repository"
    echo -e "  ${WHITE}2.${NC} Update the UPDATE_URL at the top of the script"
    echo -e "  ${WHITE}3.${NC} The script will check for new versions automatically"
    echo ""
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    
    # Try to check for updates if URL is configured
    if [[ "$UPDATE_URL" != "https://raw.githubusercontent.com/Samwelmushi/slowdns-manager/main/slowdns_script.sh" ]]; then
        ...
        if [[ -n "$REMOTE_VERSION" ]]; then
            echo ""
            echo -e "${WHITE}Latest Version: ${GREEN}v$REMOTE_VERSION${NC}"
            echo ""
            
            if [[ "$REMOTE_VERSION" != "$SCRIPT_VERSION" ]]; then
                blinking_status "New version available!" "UPDATE" "${GREEN}" 2
                echo ""
        ...
        echo -e "${CYAN}💡 Update URL not configured yet${NC}"
    fi
...
update_script() {
    echo ""
    loading_bar 3 "${CYAN}Downloading latest version${NC}"
    ...
    if curl -s "$UPDATE_URL" -o "$SCRIPT_PATH"; then
        chmod +x "$SCRIPT_PATH"
        animated_check
        echo ""
        celebrate_success "Update completed successfully!"
        echo ""
        echo -e "${YELLOW}Script will restart in 3 seconds...${NC}"
        sleep 3
...
backup_config() {
    ...
    BACKUP_DIR="/root/slowdns_backups"
    BACKUP_FILE="$BACKUP_DIR/slowdns_backup_$(date +%Y%m%d_%H%M%S).tar.gz"
    
    mkdir -p "$BACKUP_DIR"
    
    scanning_effect "${CYAN}Creating backup${NC}"
    echo ""
    
    loading_bar 3 "${CYAN}Archiving configuration files${NC}"
    
    tar -czf "$BACKUP_FILE" \
        "$INSTALL_DIR" \
        "$SSH_DIR" \
        /etc/systemd/system/dnstt.service \
        /etc/sysctl.d/99-dnstt-optimize.conf \
        2>/dev/null || true
    
    if [[ -f "$BACKUP_FILE" ]]; then
        animated_check
        echo ""
        celebrate_success "Backup created successfully!"
        echo ""
        echo -e "${WHITE}Backup location: ${GREEN}$BACKUP_FILE${NC}"
        echo -e "${WHITE}Backup size: ${CYAN}$(du -h "$BACKUP_FILE" | cut -f1)${NC}"
        echo ""
...
restore_config() {
    ...
    scanning_effect "${CYAN}Scanning for backups${NC}"
    echo ""
    ...
    for backup in "${backups[@]}"; do
        ...
        local date=$(echo "$filename" | grep -oP '\d{8}_\d{6}' | sed 's/_/ /')
        echo -e "  ${CYAN}$i)${NC} ${WHITE}$filename${NC}"
        echo -e "     Size: ${CYAN}$size${NC} | Date: ${YELLOW}$date${NC}"
        echo ""
        i=$((i + 1))
    done
    ...
    if [[ "$backup_choice" -gt 0 ]] && [[ "$backup_choice" -le "${#backups[@]}" ]]; then
        ...
        echo ""
        warning_flash "This will overwrite current configuration!"
        echo ""
        read -p "Continue? Type 'yes' to confirm: " confirm
        
        if [[ "$confirm" == "yes" ]]; then
            echo ""
            loading_bar 3 "${CYAN}Restoring configuration${NC}"
            ...
            systemctl start dnstt 2>/dev/null || true
            
            animated_check
            echo ""
            celebrate_success "Configuration restored successfully!"
        else
            echo ""
            animated_check
...
speed_test() {
    ...
    if ! systemctl is-active --quiet dnstt; then
        ...
        echo ""
        echo -e "${YELLOW}Start DNSTT service first${NC}"
        press_enter
        return
    fi
    
    echo -e "${YELLOW}Running comprehensive speed test...${NC}"
    echo ""
    
    # Test 1: Network Interface Speed
    loading_bar 2 "${CYAN}Testing network interface${NC}"
    NET_INTERFACE=$(ip route | grep default | awk '{print $5}' | head -1)
    
    if command -v ethtool &> /dev/null; then
        LINK_SPEED=$(ethtool "$NET_INTERFACE" 2>/dev/null | grep "Speed:" | awk '{print $2}')
        echo -e "  ${WHITE}Link Speed: ${GREEN}$LINK_SPEED${NC}"
    fi
    
    # Test 2: DNS Resolution Speed
    echo ""
    loading_bar 2 "${CYAN}Testing DNS resolution${NC}"
    ...
    echo -e "  ${WHITE}DNS Response: ${GREEN}${DNS_TIME}ms${NC}"
    
    # Test 3: Bandwidth Test
    echo ""
    loading_bar 3 "${CYAN}Measuring bandwidth (30 seconds)${NC}"
    ...
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                  SPEED TEST RESULTS                       ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "  ${WHITE}📥 Download: ${GREEN}${RX_MBPS} Mbps${NC}"
    echo -e "  ${WHITE}📤 Upload:   ${GREEN}${TX_MBPS} Mbps${NC}"
    echo -e "  ${WHITE}🌐 DNS:      ${GREEN}${DNS_TIME}ms${NC}"
    echo ""
    
    # Performance rating
    TOTAL_SPEED=$(echo "$RX_MBPS + $TX_MBPS" | bc)
    TOTAL_INT=$(printf "%.0f" "$TOTAL_SPEED")
    
    if (( $(echo "$TOTAL_SPEED >= 10" | bc -l) )); then
        blinking_status "Performance: EXCELLENT!" "⚡ ULTRA" "${GREEN}" 2
    elif (( $(echo "$TOTAL_SPEED >= 5" | bc -l) )); then
        blinking_status "Performance: GOOD" "✓ FAST" "${YELLOW}" 2
    else
        blinking_status "Performance: MODERATE" "○ OK" "${CYAN}" 2
    fi
...
EOF
    ...
    # Animated subtitle with country flag
    sleep 0.2
    echo -ne "\r                    "
    for i in {1..3}; do
        echo -ne "${GREEN}●${NC}"
