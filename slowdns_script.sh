...
optimize_system_ultra() {
    ...
    sysctl -w net.core.rmem_max=1073741824 > /dev/null 2>&1  # 1GB
    sysctl -w net.core.wmem_max=1073741824 > /dev/null 2>&1  # 1GB
    sysctl -w net.core.rmem_default=134217728 > /dev/null 2>&1  # 128MB
    sysctl -w net.core.wmem_default=134217728 > /dev/null 2>&1  # 128MB
    ...
    sysctl -w net.ipv4.udp_rmem_min=524288 > /dev/null 2>&1  # 512KB
    sysctl -w net.ipv4.udp_wmem_min=524288 > /dev/null 2>&1  # 512KB
    sysctl -w net.ipv4.udp_mem="524288 1048576 2097152" > /dev/null 2>&1
    
    # Advanced UDP tuning
    sysctl -w net.core.netdev_max_backlog=300000 > /dev/null 2>&1  # 300K packets
    sysctl -w net.core.netdev_budget=3000 > /dev/null 2>&1
    sysctl -w net.core.netdev_budget_usecs=20000 > /dev/null 2>&1
    sysctl -w net.core.somaxconn=262144 > /dev/null 2>&1  # 256K connections
    ...
    # Optimize for bulk transfers (SSH file transfers)
    sysctl -w net.ipv4.tcp_notsent_lowat=131072 > /dev/null 2>&1  # 128KB
    
    # Reduce retransmit time for faster recovery
    sysctl -w net.ipv4.tcp_retries1=3 > /dev/null 2>&1
    sysctl -w net.ipv4.tcp_retries2=5 > /dev/null 2>&1
    sysctl -w net.ipv4.tcp_orphan_retries=1 > /dev/null 2>&1
    ...
    sysctl -w net.netfilter.nf_conntrack_tcp_timeout_established=432000 > /dev/null 2>&1  # 5 days
    sysctl -w net.netfilter.nf_conntrack_udp_timeout=600 > /dev/null 2>&1  # 10 min
    sysctl -w net.netfilter.nf_conntrack_udp_timeout_stream=600 > /dev/null 2>&1
    ...
    sysctl -w net.ipv4.tcp_max_syn_backlog=262144 > /dev/null 2>&1
    ...
    sysctl -w net.ipv4.tcp_tso_win_divisor=3 > /dev/null 2>&1
    ...
    sysctl -w net.ipv4.ip_local_reserved_ports="" > /dev/null 2>&1
    ...
    # Increase routing cache
    sysctl -w net.ipv4.route.max_size=4194304 > /dev/null 2>&1
    echo -e "${GREEN}✓ DNS tunnel optimizations${NC}"
    
    echo -e "${CYAN}[11/12]${NC} Memory and queue optimizations..."
    # Increase socket buffer limits
    sysctl -w net.core.optmem_max=134217728 > /dev/null 2>&1  # 128MB
    
    # Increase network device transmit queue length
    sysctl -w net.core.netdev_budget=3000 > /dev/null 2>&1
    sysctl -w net.core.netdev_budget_usecs=20000 > /dev/null 2>&1
...
bandwidth_test() {
    ...
    if (( $(echo "$TOTAL_MBPS >= 10" | bc -l) )); then
        echo -e "${GREEN}✅ Performance: EXCELLENT (Target 10-25 Mbps achieved)${NC}"
    elif (( $(echo "$TOTAL_MBPS >= 5" | bc -l) )); then
        echo -e "${YELLOW}⚠️  Performance: GOOD (Consider increasing MTU)${NC}"
    else
        echo -e "${RED}❌ Performance: NEEDS IMPROVEMENT${NC}"
        echo -e "${YELLOW}   Try MTU 1420 or 4096${NC}"
    fi
