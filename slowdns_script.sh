...
test_mtu_connection() {
    ...
    if [ $MTU -le 512 ]; then
        ...
        echo -e "${WHITE}   - Use direct UDP client connection${NC}"
        echo -e "${WHITE}   - Client command: dnstt-client -udp DNS_IP:53 ...${NC}"
        echo -e "${WHITE}   - Avoid DoH/DoT for MTU 512${NC}"
    else
        echo -e "${WHITE}   - MTU $MTU is active${NC}"
        echo -e "${WHITE}   - Can use DoH/DoT for better security${NC}"
        echo -e "${WHITE}   - If issues, try lowering MTU to 512${NC}"
    fi
...
# Change MTU without reinstalling
change_mtu() {
    show_banner
    echo -e "${CYAN}╔════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║         Change MTU Configuration       ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo -e "${YELLOW}Choose new MTU:${NC}"
    echo "  1) 512  - For custom/restricted DNS"
...
User=root
WorkingDirectory=$DNSTT_DIR
ExecStart=$DNSTT_SERVER -udp :5300 -privkey-file $DNSTT_DIR/server.key -mtu $NEW_MTU $EXTRA_OPTS $tunnel_domain 127.0.0.1:$SSH_PORT
Restart=always
RestartSec=3
StandardOutput=journal
StandardError=journal
...
EOF
    ...
    echo ""
    echo -e "${GREEN}✅ MTU changed from $current_mtu to $NEW_MTU${NC}"
    echo -e "${GREEN}✅ Service restarted${NC}"
    
    if [ $NEW_MTU -le 512 ]; then
        echo ""
        echo -e "${YELLOW}📝 Note: MTU 512 requires direct UDP client:${NC}"
        echo -e "${CYAN}   dnstt-client -udp YOUR_DNS:53 -pubkey ... ${NC}"
    fi
...
dnstt_menu() {
    while true; do
        ...
        echo -e "  ${WHITE}4)${NC} ${BLUE}Change MTU Configuration${NC}"
        echo -e "  ${WHITE}5)${NC} ${CYAN}Test MTU Connection${NC}"
        echo -e "  ${WHITE}6)${NC} ${BLUE}Restart Service${NC}"
        echo -e "  ${WHITE}7)${NC} ${RED}Stop Service${NC}"
        echo -e "  ${WHITE}8)${NC} ${PURPLE}Uninstall DNSTT${NC}"
...
ssh_menu() {
    while true; do
        ...
        echo -e "  ${WHITE}5)${NC} ${PURPLE}Online Users${NC}"
        echo -e "  ${WHITE}0)${NC} Back"
        echo ""
        read -p "👉 Choice: " choice
