...
setup_dnstt() {
    ...
    echo -e "${YELLOW}This should be a subdomain that points to your NS${NC}"
    echo ""
    read -p "Tunnel domain: " tunnel_domain
    
    # If empty, create from ns_domain
    if [[ -z "$tunnel_domain" ]]; then
        # Extract base domain from ns_domain
        base_domain=$(echo "$ns_domain" | awk -F. '{print $(NF-1)"."$NF}')
        tunnel_domain="t.${base_domain}"
    fi
    
    # Clean up domain - remove any trailing dots or duplicate parts
    ...
    # Validate domains don't have double dots
    if [[ "$tunnel_domain" =~ \.\. ]]; then
        log_message "${RED}✗ Invalid domain format (contains double dots)${NC}"
        press_enter
        return 1
    fi
...
fix_domain() {
    ...
    echo -e "${YELLOW}Current broken domain:${NC}"
    cat "$INSTALL_DIR/tunnel_domain.txt"
    echo ""
    
    echo -e "${WHITE}Enter the CORRECT tunnel domain:${NC}"
    echo -e "${CYAN}Example: tun-ps7sou.firewallfalcon.thefirewoods.org${NC}"
    echo -e "${CYAN}Or: t.yourdomain.com${NC}"
    echo ""
    ...
    if [[ "$correct_domain" =~ \.\. ]]; then
        echo -e "${RED}✗ Invalid domain (contains double dots)${NC}"
        press_enter
        return
    fi
    
    # Get other settings
    MTU=$(cat "$INSTALL_DIR/mtu.txt" 2>/dev/null || echo "512")
    SSH_PORT=$(cat "$INSTALL_DIR/ssh_port.txt" 2>/dev/null || echo "22")
    
    # Save corrected domain
    echo "$correct_domain" > "$INSTALL_DIR/tunnel_domain.txt"
    
    # Recreate service
    log_message "${YELLOW}Recreating service with correct domain...${NC}"
    ...
    if systemctl is-active --quiet dnstt; then
        echo ""
        echo -e "${GREEN}✅ Fixed! Service is now running${NC}"
        echo ""
...
dnstt_menu() {
    while true; do
        ...
        echo -e "  ${PURPLE}4)${NC} Fix Domain Issue (Quick Fix)"
        echo -e "  ${BLUE}5)${NC} Restart Service"
        echo -e "  ${RED}6)${NC} Stop Service"
