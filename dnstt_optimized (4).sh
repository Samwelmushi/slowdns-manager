...
check_os() {
    ...
    if [[ -f /etc/debian_version ]]; then
        ...
        if [[ "$DEBIAN_VERSION" -lt 12 ]] 2>/dev/null; then
            warning_flash "Debian version $DEBIAN_VERSION detected"
            echo ""
            echo -e "${YELLOW}⚠️  This script is optimized for Debian 12+${NC}"
            echo -e "${YELLOW}   Some features may not work correctly on older versions${NC}"
            echo ""
            ...
            animated_check
            echo -e "${GREEN}Debian $DEBIAN_VERSION detected - Perfect!${NC}"
            sleep 1
        fi
    fi
}
...
### DEBIAN 12+ SPECIFIC OPTIMIZATIONS ###
# Additional tweaks for modern kernel
...
EOF
    ...
    echo ""
    log_success "Optimization Summary (BOOSTER + ULTRA + Debian 12+):"
    log_message "   ${GREEN}✓${NC} 256MB network buffers (ULTRA)"
    log_message "   ${GREEN}✓${NC} 128KB UDP buffers (BOOSTER - no packet loss)"
    log_message "   ${GREEN}✓${NC} 50K packet backlog (BOOSTER - DNS burst)"
    log_message "   ${GREEN}✓${NC} 65K max connections (somaxconn)"
    log_message "   ${GREEN}✓${NC} BBR congestion control (BOOSTER)"
    log_message "   ${GREEN}✓${NC} TCP FastOpen enabled (BOOSTER)"
    log_message "   ${GREEN}✓${NC} IP forwarding ON (BOOSTER - routing boost)"
    log_message "   ${GREEN}✓${NC} 1M file descriptors (BOOSTER - parallel tunnels)"
    log_message "   ${GREEN}✓${NC} 2M connection tracking"
    log_message "   ${GREEN}✓${NC} UDP port 53/5300 optimized for massive traffic"
    log_message "   ${GREEN}✓${NC} MTU probing enabled (Debian 12+)"
...
install_dependencies() {
    log_message "${YELLOW}📦 Installing dependencies for Debian 12+...${NC}"
    echo ""
    ...
    if [[ -f /etc/debian_version ]]; then
        ...
        echo ""
        loading_bar 4 "${CYAN}Installing essential packages for Debian${NC}"
        
        # Core packages for Debian 12+
        ...
        echo ""
        loading_bar 2 "${CYAN}Installing kernel modules for performance${NC}"
        ...
        cat > /etc/modules-load.d/dnstt.conf << 'EOF'
# DNSTT Required Modules
tcp_bbr
ip_tables
iptable_nat
nf_conntrack
EOF
        ...
        echo -ne "${CYAN}Installing packages for RHEL/CentOS${NC}"
    ...
    echo ""
    pulse_message "Dependencies installation completed for Debian 12+"
    sleep 1
}
...
configure_firewall() {
    ...
    # Stop and disable UFW if it exists (conflicts with our rules)
    if command -v ufw &> /dev/null; then
        ...
        animated_check
        echo "UFW disabled (using iptables directly)"
    fi
...
EOF
    ...
    loading_bar 3 "${CYAN}Configuring iptables for high-speed UDP${NC}"
    ...
    # === CRITICAL UDP PORT 53 & 5300 OPTIMIZATION ===
    # Allow massive UDP traffic on port 53 and 5300
    ...
    # === HIGH-PRIORITY UDP RULES FOR MAXIMUM SPEED ===
    # These rules are at the TOP of the chain for fastest processing
    
    # Port 5300 - DNSTT Server (CRITICAL - NO LIMITS)
    iptables -I INPUT 1 -p udp --dport 5300 -j ACCEPT
    iptables -I OUTPUT 1 -p udp --sport 5300 -j ACCEPT
    
    # Port 53 - DNS (CRITICAL - NO LIMITS)  
    iptables -I INPUT 1 -p udp --dport 53 -j ACCEPT
    iptables -I OUTPUT 1 -p udp --sport 53 -j ACCEPT
    
    # NAT rule to redirect port 53 to 5300 (AUTOMATIC PORT OPENING)
    iptables -t nat -I PREROUTING 1 -p udp --dport 53 -j REDIRECT --to-ports 5300
    
    # Allow SSH on port 22 (AUTOMATIC SSH FORWARDING)
    ...
    # Allow HTTP (for management)
    iptables -A INPUT -p tcp --dport 80 -j ACCEPT
    
    # === RATE LIMIT PROTECTION (but generous for DNS tunnel) ===
    # Allow up to 10,000 packets per second (very high for DNS tunneling)
    iptables -A INPUT -p udp --dport 5300 -m limit --limit 10000/sec -j ACCEPT
    iptables -A INPUT -p udp --dport 53 -m limit --limit 10000/sec -j ACCEPT
    
    # Connection tracking optimization for UDP
    ...
    animated_check
    echo "UDP ports 53 & 5300 opened with NO traffic limits"
    
    loading_bar 2 "${CYAN}Optimizing connection tracking for UDP${NC}"
    
    # Optimize netfilter for high UDP throughput
    ...
    # Increase conntrack hash table size for better performance
    echo 262144 > /sys/module/nf_conntrack/parameters/hashsize 2>/dev/null || true
    
    animated_check
    echo "Connection tracking optimized for 2M connections"
...
EOF
    ...
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║              FIREWALL CONFIGURATION SUMMARY               ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "  ${GREEN}✓${NC} ${WHITE}UDP Port 53:${NC}      ${GREEN}OPEN${NC} (DNS - redirected to 5300)"
    echo -e "  ${GREEN}✓${NC} ${WHITE}UDP Port 5300:${NC}    ${GREEN}OPEN${NC} (DNSTT - NO LIMITS)"
    echo -e "  ${GREEN}✓${NC} ${WHITE}TCP Port 22:${NC}      ${GREEN}OPEN${NC} (SSH - Auto forwarded)"
    echo -e "  ${GREEN}✓${NC} ${WHITE}TCP Port 443:${NC}     ${GREEN}OPEN${NC} (HTTPS)"
    echo -e "  ${GREEN}✓${NC} ${WHITE}Rate Limit:${NC}       ${GREEN}10,000 packets/sec${NC}"
    echo -e "  ${GREEN}✓${NC} ${WHITE}Connection Track:${NC} ${GREEN}2,000,000 connections${NC}"
    echo -e "  ${GREEN}✓${NC} ${WHITE}Hash Table:${NC}       ${GREEN}262,144 buckets${NC}"
    echo ""
