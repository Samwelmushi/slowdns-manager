...
generate_v2ray_config() {
    ...
    log_message "${YELLOW}📝 Generating V2Ray configuration...${NC}"
...
EOF
    ...
    echo -e "${GREEN}✓ V2Ray config created${NC}"
    log_success "V2Ray configured on port $v2ray_port"
}
...
setup_dnstt() {
    ...
    echo ""
    echo -e "${YELLOW}📱 CLIENT CONNECTION (Use your actual SSH port):${NC}"
    echo ""
    ...
    echo -e "${WHITE}  $tunnel_domain 127.0.0.1:$SSH_PORT${NC}"
    echo ""
    echo -e "${CYAN}Direct UDP (Alternative):${NC}"
    echo -e "${WHITE}dnstt-client -udp $PUBLIC_IP:5300 \\${NC}"
    echo -e "${WHITE}  -pubkey $PUBKEY \\${NC}"
    echo -e "${WHITE}  -mtu $MTU \\${NC}"
    echo -e "${WHITE}  $tunnel_domain 127.0.0.1:$SSH_PORT${NC}"
    echo ""
    echo -e "${YELLOW}💡 IMPORTANT: Use port $SSH_PORT in your app!${NC}"
    echo -e "${YELLOW}   DNSTT forwards traffic directly to SSH port $SSH_PORT${NC}"
    echo ""
...
Server IP: $PUBLIC_IP
NS Domain: $ns_domain
Tunnel: $tunnel_domain
Public Key: $PUBKEY
SSH Port: $SSH_PORT
MTU: $MTU

CLIENT COMMAND (Direct UDP - FASTEST):
dnstt-client -udp $PUBLIC_IP:5300 -pubkey $PUBKEY -mtu $MTU $tunnel_domain 127.0.0.1:$SSH_PORT
...
show_v2ray_info() {
    ...
    echo ""
    echo -e "${YELLOW}📱 STEP 1 - Start DNSTT Client:${NC}"
    echo ""
    echo -e "${GREEN}dnstt-client -udp $PUBLIC_IP:5300 \\${NC}"
    echo -e "${GREEN}  -pubkey $PUBKEY \\${NC}"
    echo -e "${GREEN}  -mtu $MTU \\${NC}"
    echo -e "${GREEN}  $tunnel_domain 127.0.0.1:$V2RAY_PORT${NC}"
    echo ""
    echo -e "${YELLOW}📱 STEP 2 - V2Ray Client Config:${NC}"
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${WHITE}Address:${NC}    127.0.0.1"
    ...
    echo -e "${WHITE}Path:${NC}       /dnstt-v2ray"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo -e "${YELLOW}💡 V2Ray Benefits:${NC}"
    echo -e "   ${GREEN}✓${NC} Better speed than direct SSH"
    echo -e "   ${GREEN}✓${NC} Lower latency with WebSocket"
    echo -e "   ${GREEN}✓${NC} More stable connections"
    echo -e "   ${GREEN}✓${NC} Better packet optimization"
    echo ""
...
STEP 1 - DNSTT Client:
dnstt-client -udp $PUBLIC_IP:5300 -pubkey $PUBKEY -mtu $MTU $tunnel_domain 127.0.0.1:$V2RAY_PORT
