...
generate_dnstt_keys() {
    ...
    # Generate 32-byte (64 hex characters) random private key - exactly like the example
    local private_key=$(openssl rand -hex 32)
    
    # The public key for DNSTT is derived from the private key
    # For DNSTT, we use the same hex key format
    ...
    log "DNSTT keys generated: $private_key"
}
