...
optimize_for_512() {
    log_message "${YELLOW}⚡ Applying 512B MTU tunnel optimizations...${NC}"
    echo ""

    # ── Explanation ─────────────────────────────────────────
    # When MTU=512, DNSTT sends/receives many small UDP packets
    # very rapidly. Normal kernel settings are tuned for large
    # packets. These tweaks maximize throughput for small, fast
    # UDP bursts — the exact pattern of a DNS tunnel at 512B.

    echo -e "${CYAN}[A]${NC} Small-packet UDP socket buffer tuning..."
    # Increase socket receive queue depth for rapid small packets
    sysctl -w net.core.rmem_max=26214400        > /dev/null 2>&1 || true
    sysctl -w net.core.wmem_max=26214400        > /dev/null 2>&1 || true
    sysctl -w net.core.rmem_default=2097152     > /dev/null 2>&1 || true
    sysctl -w net.core.wmem_default=2097152     > /dev/null 2>&1 || true
    # UDP-specific: min buffer sized for 512B * many concurrent datagrams
    sysctl -w net.ipv4.udp_rmem_min=131072      > /dev/null 2>&1 || true
    sysctl -w net.ipv4.udp_wmem_min=131072      > /dev/null 2>&1 || true
    sysctl -w net.ipv4.udp_mem="131072 262144 26214400" > /dev/null 2>&1 || true
    echo -e "${GREEN}✓ UDP socket buffers tuned for 512B small packets${NC}"
    sleep 0.3

    echo -e "${CYAN}[B]${NC} Reducing interrupt coalescing delay..."
    # netdev_budget_usecs controls how long the kernel spends
    # processing packets per NAPI poll. Lower value = faster
    # per-packet delivery for small high-frequency UDP bursts
    sysctl -w net.core.netdev_budget=600        > /dev/null 2>&1 || true
    sysctl -w net.core.netdev_budget_usecs=3000 > /dev/null 2>&1 || true
    sysctl -w net.core.netdev_max_backlog=50000 > /dev/null 2>&1 || true
    echo -e "${GREEN}✓ Interrupt budget tuned for low-latency small packets${NC}"
    sleep 0.3

    echo -e "${CYAN}[C]${NC} TCP tuning for SSH-over-512B-tunnel..."
    # The SSH session rides over TCP inside the DNSTT tunnel.
    # With 512B MTU the TCP segments are tiny — we need:
    # - tcp_nodelay equivalent: disable Nagle via low_latency
    # - smaller initial window to avoid burst drops
    # - faster ACKs to keep the pipe full
    sysctl -w net.ipv4.tcp_low_latency=1        > /dev/null 2>&1 || true
    sysctl -w net.ipv4.tcp_fastopen=3           > /dev/null 2>&1 || true
    sysctl -w net.ipv4.tcp_slow_start_after_idle=0 > /dev/null 2>&1 || true
    sysctl -w net.ipv4.tcp_sack=1              > /dev/null 2>&1 || true
    sysctl -w net.ipv4.tcp_timestamps=1        > /dev/null 2>&1 || true
    # Reduce delayed ACK timer — important for small packet tunnels
    # Default is 40ms, this halves it to 20ms
    sysctl -w net.ipv4.tcp_delack_min=0        > /dev/null 2>&1 || true
    echo -e "${GREEN}✓ TCP tuned for SSH-over-small-MTU${NC}"
    sleep 0.3

    echo -e "${CYAN}[D]${NC} Loopback interface MTU for local SSH..."
    # DNSTT forwards traffic to 127.0.0.1:SSH_PORT over loopback.
    # Loopback default MTU is 65536 — keeping it large is fine
    # but we set it explicitly to avoid any mismatch
    ip link set lo mtu 65536 2>/dev/null || true
    echo -e "${GREEN}✓ Loopback MTU set to 65536 (optimal for local forwarding)${NC}"
    sleep 0.3

    echo -e "${CYAN}[E]${NC} Saving 512B tunnel sysctl config permanently..."
    cat > /etc/sysctl.d/99-dnstt-512b-tunnel.conf << 'SYSCTL'
# DNSTT 512B MTU Tunnel Optimizations
# Created By THE KING 👑 💯
# Specific tuning for high-frequency small-packet DNS tunnels

# UDP small packet buffers
...
net.ipv4.udp_mem = 131072 262144 26214400

# Fast small-packet processing
net.core.netdev_budget = 600
net.core.netdev_budget_usecs = 3000
net.core.netdev_max_backlog = 50000

# TCP low-latency for SSH over tunnel
net.ipv4.tcp_low_latency = 1
net.ipv4.tcp_fastopen = 3
net.ipv4.tcp_slow_start_after_idle = 0
net.ipv4.tcp_sack = 1
...
SYSCTL
    ...
    echo ""
    echo -e "${GREEN}╔══════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║     ⚡ 512B MTU TUNNEL OPTIMIZATIONS APPLIED ⚡      ║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════════════╝${NC}"
    echo -e "  ${GREEN}✓${NC} UDP buffers tuned for rapid small packets"
    echo -e "  ${GREEN}✓${NC} Interrupt coalescing reduced (lower latency)"
    echo -e "  ${GREEN}✓${NC} TCP Nagle-equivalent disabled for tunnel SSH"
    echo -e "  ${GREEN}✓${NC} Delayed ACK timer minimized"
    echo -e "  ${GREEN}✓${NC} Loopback MTU maximized for local forwarding"
    echo -e "  ${YELLOW}Expected improvement: +30-60% throughput at 512B MTU${NC}"
    echo ""
    sleep 1
}
...
# Cipher list — includes BOTH modern (gcm/chacha) AND legacy (ctr) ciphers
# This fixes "No matching algorithm" errors from HTTP Injector and other clients
# that offer aes128-ctr / aes256-ctr instead of gcm variants
Ciphers aes128-ctr,aes256-ctr,chacha20-poly1305@openssh.com,aes128-gcm@openssh.com,aes256-gcm@openssh.com,aes192-ctr

# MACs — include both etm and non-etm for maximum client compatibility
MACs hmac-sha2-256-etm@openssh.com,hmac-sha2-512-etm@openssh.com,hmac-sha2-256,hmac-sha2-512

# Key exchange
KexAlgorithms curve25519-sha256,curve25519-sha256@libssh.org,ecdh-sha2-nistp256

# Tuning for DNS tunnel (512B MTU small-packet environment)
# Small RekeyLimit prevents stalls during high-frequency small packet bursts
RekeyLimit 256M 1h

# Allow more auth attempts (tunnel clients sometimes retry)
MaxAuthTries 10
EOF

    echo -e "${GREEN}✓ SSH server optimized — cipher mismatch fix applied${NC}"
    echo -e "${GREEN}✓ aes128-ctr + aes256-ctr added (HTTP Injector compatible)${NC}"
